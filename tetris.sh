...
#位置与大小
#屏幕的左上角那一点距离棋盘左上角这一点的距离（左距离和上距离）#
marginLeft=3			#边框左边距
marginTop=2			#边框上边距
#棋盘内左上角那一点距离屏幕左边和上边的距离#
((mapLeft=marginLeft+2))	#棋盘左边距
((mapTop=$marginTop+1))		#棋盘上边距
#棋盘的宽度是10不意味这棋盘的宽度是10个字符，而是棋盘的方块数是10个，实际上每个方块有两个字符构成的#
#也就是说这里面实际上是20个字符#
mapWidth=10			#棋盘宽度
mapHeight=15			#棋盘高度
...
#方块定义，7大类19种样式，每种类型通过旋转又可以有自己的一个不同的样式
#前8位为方块坐标，后2位为方块刚出现的时候的位置，方块（大方快，小方块放到16格的大方快中）的左上角的这个位置距离棋盘的位置
#每个坐标，前面那个数字是y,后面那个数字是x,先行后列#
#正方形方块是不能够旋转的，只有一种样式#
box0_0=(0 0 0 1 1 0 1 1 0 4)

#竖线和横线，他们可以根据1，1这个坐标来进行旋转#
